...
[task_local]
# 连续签到每日版_part5
0 4 * * *  t_countinue_sign_everyday_part5.js, tag=连续签到每日版_part5, enabled=true
 */
const $ = new Env('连续签到每日版_part5');
...
!(async () => {
    ...
    cklen = cookiesArr.length > 15 ? 15 : cookiesArr.length
    if (cklen <= 12) {
        return
    }
    for (let i = 12; i < cklen; i++) {
        if (cookiesArr[i]) {
            cookie = cookiesArr[i];
