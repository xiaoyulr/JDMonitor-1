...
[task_local]
# 连续签到每日版_part2
0 1 * * *  t_countinue_sign_everyday_part2.js, tag=连续签到每日版_part2, enabled=true
 */
const $ = new Env('连续签到每日版_part2');
...
!(async () => {
    ...
    cklen = cookiesArr.length > 6 ? 6 : cookiesArr.length
    if (cklen <= 3) {
        return
    }
    for (let i = 3; i < cklen; i++) {
        if (cookiesArr[i]) {
            cookie = cookiesArr[i];
