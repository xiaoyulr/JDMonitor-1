...
async function jdmodule() {
    ...
    await takePostRequest("getSystemConfig")
    ...
    await takePostRequest("findSkus")
    ...
    switch ($.domain) {
        case 'lzkj-isv.isvjcloud.com':
            ...
            break;
        case 'cjhy-isv.isvjcloud.com':
            await takePostRequest("getSignInfo2")
            break;
        default:
    };
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'getSystemConfig':
            url = `https://${$.domain}/wxCommonInfo/getSystemConfig`;
            body = `activityId=${$.activityId}&activityType=`;
            break;
        ...
        case 'accessLogWithAD':
            ...
            pageurl = `${$.activityUrl}&friendUuid=${$.friendUuid}`
        ...
        case 'accessLog':
            ...
            pageurl = `${$.activityUrl}&friendUuid=${$.friendUuid}`
            body = `venderId=${$.venderId}&code=15&pin=${encodeURIComponent($.Pin)}&activityId=${$.activityId}&pageUrl=${encodeURIComponent(pageurl)}&subType=app`
            break;
        case 'getSignInfo':
        ...
        case 'getShopInfo':
            ...
            break;
        case 'findSkus':
            url = `https://${$.domain}/act/common/findSkus`;
            body = `actId=${$.activityId}&userId=${$.venderId}&type=15`
            break;
        case 'signUp':
    ...
    return new Promise(async resolve => {
        $.post(myRequest, (err, resp, data) => {
            // console.log(`${type}-->resp:` + JSON.stringify(resp.headers))
            try {
                if (type != 'getActivity' || type != 'accessLog' || type != 'getOpenStatus' || type != 'getSystime') {
            ...
            case 'getActivity':
                if (typeof res == 'object') {
                    if (res.isOk && res.isOk === true) {
                        act = res.act
                        wsagb = act.wxSignActivityGiftBean
                        // console.log(JSON.stringify(act))
                        giftConditions = wsagb.giftConditions
                        for (let giftCondition of giftConditions) {
                            if (giftCondition.gift != null && (giftCondition.gift.giftType != `2` || giftCondition.gift.giftType != `9`)) {
                                $.signFlag = true
                            }
                        }
                    } else if (res.errorMessage) {
...
function getPostRequest(url, body, method = "POST") {
    ...
    if (url.indexOf($.domain) > -1) {
        ...
        headers["Cookie"] = `IsvToken=${$.Token};${lz_jdpin_token_cookie && lz_jdpin_token_cookie || ''}${$.Pin && "AUTH_C_USER=" + $.Pin + ";" || ""}${activityCookie}${$.lz_aes_pin}${$.app_abbr}`
    ...
    if (setcookies) {
        ...
        for (let ck of setcookie) {
            ...
            if (name.split("=")[1]) {
                ...
                if (name.indexOf('APP_ABBR=') > -1) $.app_abbr = name.replace(/ /g, '') + ';'
            }
        }
    }
