...
[task_local]
# 粉丝互动
7 7 7 7  t_fans_inter_everyday.js, tag=粉丝互动, enabled=true
 */
...
$.activityUrlArrays = process.env.T_FANS_INTER_URL_ARRAY ? process.env.T_FANS_INTER_URL_ARRAY : "";
$.Token = "";
$.openCard = false
$.friendUuid = ""
...
async function jdmodule() {
    ...
    console.log("开始做互动任务，任务名为：" + $.actName)

    if ($.isGameEnd) {
        $.putMsg(`${$.actName} 活动已结束`)
        stop = true;
        return;
    }
...
async function run() {
    try {
        for (let taskId of $.taskIds.split(",")) {
            if (taskId == "1") {
                if ($.task1Sign.finishedCount < $.task1Sign.upLimit) {
                    console.log("开始签到...")
            ...
            if (taskId == "2") {
                if ($.task2BrowGoods.finishedCount < $.task2BrowGoods.upLimit) {
                    console.log("开始浏览商品...")
                    let finishedCount = $.task2BrowGoods.finishedCount
                    for (let product of randomArray($.task2BrowGoods.taskGoodList)) {
                        if (finishedCount == $.task2BrowGoods.upLimit) {
                            break;
                        }
                        if (product.finished) {
                            continue;
            ...
            if (taskId == "3") {
                if ($.task3AddCart.finishedCount < $.task3AddCart.upLimit) {
                    console.log("开始加购商品...")
                    let finishedCount = $.task3AddCart.finishedCount
                    for (let product of randomArray($.task3AddCart.taskGoodList)) {
                        if (finishedCount == $.task3AddCart.upLimit) {
                            break;
                        }
                        if (product.finished) {
                            continue;
            ...
            if (taskId == "6") {
                if ($.task6GetCoupon.finishedCount < $.task6GetCoupon.upLimit) {
                    console.log("去获取优惠券...")
                    let finishedCount = $.task6GetCoupon.finishedCount
                    for (let coupon of randomArray($.task6GetCoupon.taskCouponInfoList)) {
                        if (finishedCount == $.task6GetCoupon.upLimit) {
                            break;
                        }
                        if (coupon.finished) {
                            continue;
                        }
                        $.couponId = coupon.couponInfo.couponId
                        await takePostRequest("doGetCouponTask")
                        finishedCount++
                        await $.wait(parseInt(Math.random() * 2000 + 1000, 10))
                    }
                    if (finishedCount < $.task6GetCoupon.finishedCount) {
                        console.log("优惠券不足，该任务可以废弃了...")
                    }
                } else {
                    console.log("获取优惠券已经完成啦~")
                }
            }
            if (taskId == "7") {
...
async function takePostRequest(type) {
    ...
    switch (type) {
        ...
        case 'accessLogWithAD':
            ...
            let pageurl = `https://${$.domain}/drawCenter/activity?activityId=${$.activityId}`
        ...
        case 'activityContent':
            url = `https://${$.domain}/wxFansInterActionActivity/activityContent`;
        ...
        case 'getActMemberInfo':
            ...
            break;
        case 'doSign':
            url = `https://${$.domain}/wxFansInterActionActivity/doSign`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}`
            break;
        case 'doShareTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doMeetingTask`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}`
            break;
        case 'doRemindTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doRemindTask`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}`
            break;
        case 'doGetCouponTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doGetCouponTask`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}&couponId=${$.couponId}`
            break;
        case 'doBrowGoodsTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doBrowGoodsTask`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}&skuId=${$.skuId}`
            break;
        case 'doAddGoodsTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doAddGoodsTask`;
            body = `activityId=${$.activityId}&uuid=${$.uuid}&skuId=${$.skuId}`
            break;
        case 'doMeetingTask':
            url = `https://${$.domain}/wxFansInterActionActivity/doMeetingTask`;
    ...
    try {
        switch (type) {
            ...
            case 'activityContent':
                if (typeof res == 'object') {
                    if (res.result && res.result === true) {
                        let actInfo = res.data.actInfo
                        if (typeof actInfo == 'object') {
                            $.activityName = actInfo.actName
                            $.tplId = actInfo.tplId
                            $.needFollow = actInfo.needFollow
                            $.actRule = actInfo.actRule
                            $.taskIds = actInfo.taskIds
                            $.giftLevelOne = actInfo.giftLevelOne
                            $.giftLevelTwo = actInfo.giftLevelTwo
                            $.giftLevelThree = actInfo.giftLevelThree
                            $.actName = actInfo.actName
                        }
                        let actorInfo = res.data.actorInfo
                        if (typeof actorInfo == 'object') {
                            if ($.index == 1) {
                                $.friendUuid = actorInfo.uuid
                                console.log(`本次任务助力ID:${$.friendUuid}`)
                            }
                            $.uuid = actorInfo.uuid
                            $.curEnergy = actorInfo.energyValue
                            $.prizeOneStatus = actorInfo.prizeOneStatus
                            $.prizeTwoStatus = actorInfo.prizeTwoStatus
                            $.prizeThreeStatus = actorInfo.prizeThreeStatus
                        }
                        $.task1Sign = res.data.task1Sign
                        $.task2BrowGoods = res.data.task2BrowGoods
                        $.task3AddCart = res.data.task3AddCart
                        $.task4Share = res.data.task4Share
                        $.task5Remind = res.data.task5Remind
                        $.task6GetCoupon = res.data.task6GetCoupon
                        $.task7MeetPlaceVo = res.data.task7MeetPlaceVo
                        $.canDrawFlg = res.data.canDrawFlg
                    } else if (res.errorMessage) {
                        console.log(`${type} ${res.errorMessage || ''}`)
                    } else {
                        console.log(`${type} ${data}`)
